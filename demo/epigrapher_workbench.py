...
def main():
    ...
    if args.test_lacuna:
        ...
        scores = engine.score_rival_proposals(
            left, right, proposals, min_chars=3, max_chars=5
        )
        for rank, s in enumerate(scores, 1):
            valid_tag = (
                "VALID" if s.is_spatially_valid else "INVALID (Spatial Bound Violation)"
            )
            print(
                f"Rank {rank}: [{s.attribution}] '{s.proposal}' — LogProb: {s.log_prob}, SpatialFit: {s.spatial_fit} ({valid_tag})"
            )
