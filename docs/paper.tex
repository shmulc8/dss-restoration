...
On a shared 729-word diagnostic, TavBERT base obtains 21.1\% Hit@10, while DSS fine-tuning does not improve it. MsBERT's aligned-only Hit@10 is 21.6\%, but 279 alignment failures reduce its all-target rate to 13.3\%. Alignment failures therefore remain misses in every subsequent comparison; the full diagnostic is preserved in the released artifact.
...
On the 68 QD targets with composition labels, removing all matching compositions from seed-42 training leaves context, soft-trace, and hard-trace Top-10 unchanged at 11.8\%, 52.9\%, and 50.0\%. Soft-trace Top-10 also remains 55.9--57.0\% across context windows. No attributed reading occurs in an exact preserved-training trigram or five-gram, although base-model pretraining exposure cannot be excluded. Detailed calibration and sensitivity artifacts accompany the release.
