...
\begin{abstract}
\normalsize
Dead Sea Scroll lacunae differ from ordinary masked-word tasks: visible letters may survive, while character length, word count, and boundaries remain uncertain. We introduce a reconstruction-free corpus derivative and two complementary evaluations that keep these information sources explicit. On 93 natural single-word lacunae, a preserved-only masked language model attains 15.1\% Top-10 agreement with attributed proposals from context alone and 50.5\% when supplied transcription-visible letters; a frequency baseline with the same traces reaches 36.6\%. On 300 non-overlapping synthetic spans sampled across 79 held-out scrolls, the strongest unknown-length decoder reaches 7.7\% exact Top-10 and recovers no three-word target. These results establish a useful but bounded conclusion: transcription evidence sharply narrows candidate sets, whereas present language models do not solve unknown-length multiword restoration. Agreement with published proposals is not historical ground truth.
\end{abstract}
...
\begin{itemize}
  ...
  \item a paired 93-target ablation with a frequency baseline, candidate-coverage reporting, manuscript-cluster uncertainty, and a composition-unseen subset; and
...
\node[stage,fill=DSSEmerald!10,below=of gen] (constraint) {\textbf{Declared evidence condition}\\ none; visible traces; editor-length oracle};
...
We distinguish four information conditions:
...
We report 95\% bootstrap intervals clustered by scroll and paired cluster intervals for evidence-condition deltas. Target-level predictions, exclusions, manifests, and code/data/model/result hashes accompany the aggregate tables. ByT5 uses three matched seeds; the MLM and TavBERT comparisons remain single-checkpoint analyses and are labeled accordingly. These choices follow reproducibility guidance in NLP \cite{dodge2019,ulmer2024}.
...
\addplot[draw=DSSNavy,fill=DSSNavy!76] coordinates
  {(FreqC,3.23) (MLMC,15.05) (FreqV,36.56) (MLMV,50.54)};
\end{axis}
\end{tikzpicture}%
}
\caption{QD Top-10 literature agreement on a fixed 93-target denominator. Trace filters help both systems; the MLM retains a 14.0-point advantage over frequency under the same visible evidence.}
\label{fig:qd-bars}
\end{figure}

\paragraph{Visible transcription traces carry most of the QD signal.}
On the same 93 targets, visible-trace filtering changes MLM Top-10 agreement from 15.1\% to 50.5\% (Table~\ref{tab:qd}). The paired improvement is 35.5 points, with a 24.5--46.5 manuscript-cluster 95\% interval. The absolute visible-trace score has a 40.3--60.0\% interval. Under the same traces, the frequency baseline reaches 36.6\%; the paired MLM advantage is 14.0 points with a 3.7--23.1 interval. Constraint narrowing therefore explains part, but not all, of the result. Coverage is imperfect: the MLM filter returns no candidate for 13 targets and fewer than ten for 37, all retained as misses. The editor-derived length proxy adds no Top-10 gain over visible traces alone and is not used in the headline.

\paragraph{Trace-count error analysis.}
Visible conditioning is not uniformly beneficial. For the 12 targets with only one transcribed letter, Top-10 falls from 41.7\% context-only to 16.7\%; for 14 two-letter targets it rises from 28.6\% to 57.1\%, and for 67 targets with at least three letters it rises from 7.5\% to 55.2\%. Single-letter placement is therefore too brittle to treat as a reliable hard filter in this sample. A production system should soften uncertain traces and expose conflicts rather than silently discarding candidates.

\paragraph{Composition-unseen analysis.}
The frozen split is manuscript-disjoint but not composition-disjoint. Restricting QD to 24 targets from 11 scrolls whose composition labels do not occur in train or development yields 16.7\% context-only and 62.5\% visible-trace Top-10; the matched frequency baseline reaches 33.3\%. This small, selected subset is a robustness check, not a substitute for training on a composition-grouped split.
...
Core paper-facing numbers are regenerated from a frozen result snapshot containing target-manifest, model, input, and artifact hashes; a machine-readable table specification records evidence class and sources. The snapshot reproduces the 300-span baselines and three matched ByT5 evaluations on one target hash, and reruns QD over 93 target-level records. The preserved corpus validator enforces non-biblical membership, removal of modern reconstruction text, anonymous gap representation, and disjoint scrolls. A separate audit records composition overlap, exact duplicates, and five-gram similarity.
...
The QD model is a single training seed; its interval captures target and manuscript sampling, not optimization variance. The composition-unseen subset is small, and composition labels are incomplete. The synthetic benchmark covers only one to three words and two context words per side. Character models solve alignment but have not solved exact multiword generation. Direct claims about ink require image inputs and image/transcription registration, which are absent here.
...
Transcription-visible letters substantially narrow candidate space, and a contextual MLM improves over frequency under the same evidence. This is the paper's positive result. Its equally important negative result is that token alignment and granularity ensembling do not solve unknown-length multiword restoration: exact recovery approaches zero beyond one word. Shared denominators, coverage accounting, manuscript-balanced sampling, clustered uncertainty, and explicit oracle labels make both conclusions reproducible without turning literature agreement into historical truth.
