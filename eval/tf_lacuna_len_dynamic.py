...
def eval_model(repo, nice):
    ...
    for ctx, gap_pos, golds, N in sample:
        ...
        for target_idx, gold in enumerate(golds):
            ...
            ids_slot = torch.cat(
                [
                    ids_slot[:target_start],
                    torch.tensor([MASK]),
                    ids_slot[target_ps[-1] + 1 :],
                ]
            )

            ranked_slot = beam_autoregressive_dynamic(
                model, ids_slot, 1, tok, beam_width=5
            )
            ng = norm(gold)

            flat_ranked_slot = [cand[0] if cand else "" for cand in ranked_slot]
            rank = next(
                (i for i, r in enumerate(flat_ranked_slot) if norm(r) == ng), 999
            )
...
with open(results_file, "w", encoding="utf-8") as f:
    ...
    f.write(
        "This table shows the realistic performance of `BEREL base` and `BEREL ft-SPAN` when evaluated **without leaking subtoken length** (using dynamic decoding).\n\n"
    )
