    ...
    for step in range(len(gp)):
        ...
        for score, current_ids, filled, pred_words in beams:
            ...
            for slot_score, seq in slot_beams:
                ...
                new_beams.append(
                    (score + slot_score, new_ids, new_filled, new_pred_words)
                )
...
def eval_model(repo, nice):
    ...
    for ctx, gap_pos, golds, N in sample:
        ...
        ranked_l2r = beam_autoregressive_left_to_right(
            model, ids, gp, tok, beam_width=5
        )
        rank_l2r = 999
        for i, cand in enumerate(ranked_l2r):
            if len(cand) == len(gold_seq) and all(
                norm(c) == g for c, g in zip(cand, gold_seq)
            ):
                rank_l2r = i
                break
...
with open(dest_file, "w", encoding="utf-8") as f:
    ...
    f.write(
        "Comparative evaluation of **Left-to-Right Sequential Beam Search** vs. our new **Confidence-Based (Easy-First) Beam Search**.\n\n"
    )
    ...
    for nice, l2r_cells, easy_cells in res:
        ...
        for metric, mi in [
            ("Top-1 Sequence Accuracy", 0),
            ("Top-10 Sequence Accuracy", 2),
        ]:
            f.write(f"### {metric}\n\n")
            ...
            for b in order:
                c = l2r_cells.get(b)
                cells_l2r.append(
                    f"{(c[mi] / c[4] * 100 if c and c[4] else 0):.1f}%" if c else "-"
                )
