...
from experiments.cross_corpus_connections import Passage, source_receipt  # noqa: E402
from experiments.cross_corpus_retrieval_ablation import (  # noqa: E402
    CONTEXT_WORDS,
    DEV_PER_LENGTH,
    MAX_CHARS,
    MAX_WORDS,
...
from experiments.tf_embible_dss_benchmark import (  # noqa: E402
    Item,
    deduplicate,
    fit_penalty,
    rank_with_penalty,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--candidate-cache",
        type=Path,
        default=ROOT / "comparison" / "cache" / "external_retrieval_candidates.json",
    )
    parser.add_argument(
        "--injection-cache-dir",
        type=Path,
        default=ROOT / "comparison" / "cache" / "retrieval_injection",
    )
    parser.add_argument(
        "--bhsa-path",
        type=Path,
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=ROOT / "comparison" / "reports" / "cross_corpus_retrieval_injection.json",
    )
    parser.add_argument(
        "--output-markdown",
        type=Path,
        default=ROOT / "comparison" / "reports" / "CROSS_CORPUS_RETRIEVAL_INJECTION.md",
    )
