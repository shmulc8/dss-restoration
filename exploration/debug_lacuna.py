...
for repo, nice in [
    ...
    print(f"### {nice}")
    print(
        f"  gold OOV(→UNK): {oov / len(items) * 100:.0f}%   single-token gold: {single / len(items) * 100:.0f}%"
        f"   avg subtokens/gold: {subtok_sum / len(items):.2f}"
    )
    print(
        f"  ALL words     EXACT top1={ex1 / n * 100:.1f} top10={ex10 / n * 100:.1f} | "
        f"NORM top1={lo1 / n * 100:.1f} top10={lo10 / n * 100:.1f}   (n={n})"
    )
    print(
        f"  CONTENT (raw) EXACT top1={c_ex1 / cn * 100:.1f} top10={c_ex10 / cn * 100:.1f} | "
        f"NORM top1={c_no1 / cn * 100:.1f} top10={c_no10 / cn * 100:.1f}   (n={cn})"
    )
    print(
        f"  CONTENT (filt)EXACT top1={cf_ex1 / cn * 100:.1f} top10={cf_ex10 / cn * 100:.1f} | "
    ...
    for gold, top5 in examples:
        hit = (
            "EXACT"
            if top5 and gold == top5[0]
            else (
                "in5"
                if gold in top5
                else ("~norm" if any(norm(t) == norm(gold) for t in top5) else "")
            )
        )
