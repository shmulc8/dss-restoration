...
def eval_model(repo, nice):
    ...
    for ctx, tpos, gold in items:
        ...
        c_cond[0] += rank_cond == 0
        c_cond[1] += rank_cond < 5
        c_cond[2] += rank_cond < 10
        c_cond[3] += 1
...
print(
    f"{'model':22s} | {'unconstrained (top-1/5/10)':>28s} | {'length-conditioned (top-1/5/10)':>32s}"
)
for repo, nice in MODELS:
    ...
    nc = max(cc[3], 1)
    print(
        f"{nice:22s} | {cb[0] / nb * 100:5.1f}% / {cb[1] / nb * 100:5.1f}% / {cb[2] / nb * 100:5.1f}% | {cc[0] / nc * 100:5.1f}% / {cc[1] / nc * 100:5.1f}% / {cc[2] / nc * 100:5.1f}%"
    )
