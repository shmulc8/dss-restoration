...
OUT_DIR = HERE / "merged_results" / sys.argv[2]
TOP_K = 10
NUM_BEAMS = int(sys.argv[3]) if len(sys.argv) > 3 else 10
OLEN = len(sys.argv) > 4 and sys.argv[4] == "olen"
    ...
    for span in sorted(spans_by_uid.get(uid, []), key=lambda s: s["span_index"]):
        ...
        for seq, score in ranked:
            ...
            if not text or text in seen:
                continue
            if OLEN and not (gold_len - 2 <= len(text) <= gold_len + 2):
                continue
            if len(cands) >= TOP_K:
                break
            seen.add(text)
