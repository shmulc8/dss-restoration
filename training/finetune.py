...
BASES = [
    ("dicta-il/BEREL", repo_path("ft_berel")),
    ("dicta-il/MsBERT", repo_path("ft_msbert")),
]
MAX_LEN = 160
EPOCHS = 4
BATCH = 16
...
print(
    f"device={dev} | train chunks={len(texts)} | ~{sum(len(t.split()) for t in texts):,} words\n"
)
...
def finetune(repo, outdir):
    ...
    for ep in range(EPOCHS):
        ...
        for si in range(steps_per_epoch):
            ...
            tot += out.loss.item()
        print(
            f"  epoch {ep + 1}/{EPOCHS}  loss={tot / steps_per_epoch:.3f}", flush=True
        )
