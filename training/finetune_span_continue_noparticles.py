"""Continue MsBERT span-ft training while excluding 1-character targets.

Starts from the already-trained ft_msbert_span checkpoint instead of from the
base MsBERT model. This is a gentler intervention than training a fresh model
...
def count_eligible_words():
    ...
    print(
        f"eligible target words: {eligible_words}/{total_words} ({eligible_words / max(total_words, 1) * 100:.1f}%)"
    )
